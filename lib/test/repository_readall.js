...
describe("When using repository to read all", () => {
    ...
    it("should cast an exception if a unsolvable expression is used", () => {
        assert.throws(() => {
            repository.exposeFilters(new enumerable_1.default().where((car) => this.unknown == 5));
        }, Error);
    });
    it("should intersection filter properties that is common ", () => {
        var list = repository.exposeFilters(new enumerable_1.default().where((car) => (car.registrationYear == 2015 && car.location == 'NO') || car.registrationYear == 2015 || (car.location == 'SE' && car.registrationYear == 2015)));
    ...
    it("should handle method calls", () => {
        ...
        if (Date.now() < new Date(2017, 1, 1).getTime())
            return;
        assert.ok(list.groups.length == 1, "Expected a single filter");
        assert.ok(list.groups[0].criteria.length == 1, "Expected a single criteria");
    });
